...
\lhead{Introduction to AI - 05.2\_logistic - Conor}
\rhead{\thepage}
\cfoot{}
...
\begin{center}
  \includegraphics[]{05_2_sneak.png}
\end{center}
  ...
  this:
\begin{center}
  \includegraphics[]{05_2_fraction.png}
  \end{center}
...
\begin{center}
  \includegraphics[]{05_2_logistic.png}
\end{center}
...
\end{equation}
so $p(x_i)$ is the `true probability' for success when $x=x_i$ and our
...
\end{equation}
where $y_i=1$ for success and $y_i=0$ for failure, the overall
likelihood is
\begin{equation}
...
this curve to make predictions about how people speak today. I
...
they said ``snuck'' for sure, that they were aware of `sneaked' as an
...
linear term in a Taylor expansion; we can also point to the emperical
...
out nicely using the logistic function and emperically we know
...
\begin{equation}
  \mathcal{E}=-\log{L}=-\sum_{y_i=1}\log\hat{p}(\mathbf{x}_i)-\sum_{y_i=0}\log[1-\hat{p}(\mathbf{x}_i)]
\end{equation}
