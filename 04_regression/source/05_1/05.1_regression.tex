...
\lhead{Introduction to AI - 05.1\_regression - Conor}
\rhead{\thepage}
\cfoot{}
...
\begin{center}
  \includegraphics[ext=.png]{05_1_points.png}
  \end{center}
...
\begin{center}
  \includegraphics[ext=.png]{05_1_points_line.png}
 \end{center}
...
that approximate and our measurement of the position and speed will not be
...
very inconvenient because the absolute value can be an awkward object;
it has long been common to use the \textsl{root mean square error}
instead:
\begin{equation}
...
\end{equation}
or something based on information theory, is hiding some idea about
what sort of processes we think are at work in generating the data, or
what so or errors we can tolerate best when using the model. In fact,
along with the practical impetus to using $\mathcal{E}_2$, there is a
principled argument based on models of the noise and the nature of
prediction which describes reasonably generic circumstances where it
is the best choice. We will not go into that here.
...
if $f(x)=f(u(x))$ then the rate of change of $f$ with $x$ is the
...
Note that $m$ depends on the sample average of the auto- and
cross-correlation; it could equally well have been written using
different notations:
\begin{equation}
m
= \frac{\langle (x_i-\bar{x})(y_i-\bar{y})\rangle_i}{\langle (x_i-\bar{x})^2\rangle_i}
= \frac{\mathbb{E}_i\!\left[(x_i-\mathbb{E}_i[x_i])(y_i-\mathbb{E}_i[y_i])\right]}{\mathbb{E}_i\!\left[(x_i-\mathbb{E}_i[x_i])^2\right]}
= \frac{\operatorname{Cov}(x,y)}{\operatorname{Var}(x)} .
\end{equation}
It can seem annoying to have all these different notations, but it is
useful to be comfortable with changing notation, partly because
different people use different systems aof notation, but also because
different notations represent different insights and different ways to
think about what is happening. Here, the angle brackets notation comes
from a more geometric tradition, emphasising projection of the data
down to different directions, the expectation values and so represent
a statistical approach. It is all the same mathematics though.

Anyway, lets do the example above! There are eight samples with
\begin{equation}
...
\begin{center}
  \includegraphics[ext=.png]{05_1_contour.png}
  \end{center}
...
The contours are
\begin{center}
  \includegraphics[ext=.png]{05_1_simple_contour.png}
  \end{center}
...
\begin{center}
  \includegraphics[ext=.png]{05_1_gradient.png}
  \end{center}
...
\begin{center}
  \includegraphics[ext=.png]{05_1_gradient_big_eta.png}
  \end{center}
