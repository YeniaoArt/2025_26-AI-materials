...
\lhead{Introduction to AI - 06.3\_training - Conor}
\rhead{\thepage}
\cfoot{}
...
means networks with many layers and the role deepness plays in this
...
these parameters will be weights and biases for a neural
...
vector\footnote{I have previously owned up to the inconsistency of how I use indices; computer scientists reasonably don't use superscipts so much since in computer code indices are indices are indices!}, so the first data point in the list is
\begin{equation}
...
will be \textsl{one hot vectors}\footnote{Almost all of machine
learning for classification uses one hot vectors; at first it seems
proflicate, allowing denser coding in the outside later would result
in much shorter output vectors, indeed logarithmically shorter, but
this doesn't work, the corresponding outputs are just much harder to
learn and the idea of considering the output as giving probabilities
to compare to the one-hot target output would be very complicated for
a dense representation. In practice, the neural network learns a dense
representation itself for the layer immeadiately before the output
layer and a matrix of weights translating this the typically very
large output vector.}, which means they will have a single one value
corresponding to the class of the item and zeros for all the other
entries, so if the first item in the data set belongs to class $k$
then
\begin{equation}
...
\begin{figure}[htb]
\begin{center}
  \includegraphics[width=3in]{06_3_objective.png}
\end{center}
...
which actually seems to make the optimization more robust; as a
side-note the idea that SGD helps you avoid getting stuck is probably
a small bit misleading, the gradient flow proceeds in a very
high-dimensional space and so local minima are probably rare compared
to narrow and tortuous valleys and the stochastic element is probably
most useful in avoiding those; so the cartoon in the figure is like a
slice through the high-dimensional space and the point holds even if
the directions are at minima.


\begin{figure}[htb]
\begin{center}
  \includegraphics[width=3in]{06_3_objective_noise.png}
\end{center}
...
This is not at all the end of the story. One obvious problem is that
you might expect the gradients to be very low when you are far from
the true solution; two different very wrong answers will produce more
or less the same error, whereas the gradient will be high when we are
close to the minimum. This is exactly what we don't want, it means we
move slowly when we are far away and precision isn't required and
quickly when we are close and it is. This implies some sort of
dynamical adjustment should be made to the learning rate and, indeed,
there are many approaches to that, including the every popular ADAM
optimiser; though, strictly, ADAM does more than that by including
contributions from the second derivative. A lot of current effort in
machine learning is focused on issues of how to do learning in an
efficient, stable and useful way: adjusting learning steps and
rescaling weights so that some don't `take over' by become too large,
for example.
